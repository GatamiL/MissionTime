...
{
    public partial class DepartmentReportWindow : Window
    {
        private readonly DbService _db;
        public DepartmentReportWindow(DbService db, int currentYear, int currentMonth)
        {
            InitializeComponent();
            _db = db;
        ...
        private void cbDepartment_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
        {
            ...
            if (cbDepartment.SelectedItem is ActiveDepItem dep)
            {
                bool hasSig = ExcelImageHelper.HasSignature(_db, dep.ResponsibleId);
                chkSign.IsEnabled = hasSig;
                chkSign.IsChecked = hasSig; // Автоматически включаем, если нашли
            }
        }
        ...
        {
            // Достанем главную ViewModel, чтобы дернуть статус-бар в основном окне
            ...
            try
            {
                ...
                {
                    mainVm.IsBusy = true;
                    mainVm.StatusText = "Формирование Excel-отчета по отделу...";
                }
                ...
                string templatePath = System.IO.Path.Combine(templatesDir, "DepartmentReportTemplate.xlsx");
                string depName = MissionTime.Services.ExcelDepartmentUtils.SanitizeFileName(((ActiveDepItem)cbDepartment.SelectedItem).Name);
                string progName = MissionTime.Services.ExcelDepartmentUtils.SanitizeFileName(((ProgItem)cbProgram.SelectedItem).ShortName);
                ...
                {
                    MissionTime.Services.ExcelDepartmentUtils.GenerateReport(
                        templatePath, 
                        outPath, 
                        _db, 
                        progId, 
                ...
                if (mainVm != null) mainVm.StatusText = "Отчет по отделу готов";
